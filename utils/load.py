...
from nacp_parser import NacpDeclarationParser, NacpDeclarationNewFormatParser, parse_guid_from_fname
...
def import_batch(docs, db_config, parser_type):
    ...
    for doc_file in docs:
        ...
        elif parser_type == "nacp_new_format":
            doc_object = NacpDeclarationNewFormatParser.parse(doc_file)
...
def import_all(
    docs_dir, corrected_file, db_config, concurrency, chunks_per_process, state_filename=None, parser_type="nacp"
):
    assert parser_type in ("nacp", "nacp_new_format")
    ...
    # For now we are rigging both parsers
    NacpDeclarationParser.corrected = corrected
    NacpDeclarationParser.translator = translator
    NacpDeclarationNewFormatParser.corrected = corrected
    ...
    with Pool(concurrency) as pool:
        ...
        results = pool.imap(
            partial(import_batch, db_config=db_config, parser_type=parser_type),
            grouper(filter(lambda x: parse_guid_from_fname(x)[0] not in all_uuids, jsons), chunks_per_process),
        )
        for i, result in enumerate(results):
            ...
            rates.append(rate)
            logger.info(
                "Batch #{} finished importing {} new documents in {:.2f}s with the rate of {:.2f}docs/s".format(
                    i, len(imported_docs), time_spent, rate
                )
            )
    ...
    if total_imported_docs:
        logger.info(
            "Total imported {} new documents with the avg rate of {:.2f}docs/s across all processes".format(
                total_imported_docs, sum(rates) / float(len(rates)) * concurrency
            )
        )
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-t", "--parser_type", help="Type of NACP parser", choices=("nacp", "nacp_new_format"), default="nacp"
    )
    args = parser.parse_args()

    db_config = {"user": args.username, "password": args.password, "url": args.endpoint, "name": args.dbname}
    ...
    import_all(
        ...
        corrected_file=args.corrected_file,
        db_config=db_config,
        concurrency=args.concurrency,
        chunks_per_process=args.chunks,
