...
def merge_csv(
    ...
    inputs,
    on_field,
    nan_replacements=None,
    postprocess_funcs=None,
    only_years=None,
    type_overrides=None,
):
    ...
    df = pandas.read_csv(inputs[0], dtype=type_overrides)
    logger.info('Base file: "{}"'.format(inputs[0]))
    for input_file in inputs[1:]:
        next_input = pandas.read_csv(input_file, dtype=type_overrides)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "files", help="CSV files. First one is base for others.", nargs="+"
    )
