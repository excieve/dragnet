(doc) => {
    ...
    function remove_not_applicable(s) {
        s = s || "";
        if (s == "[Не застосовується]") {
            s = "";
        }
        return s;
    }
    ...
    const dcu_link = `https://declarations.com.ua/declaration/${doc.doc_uuid}`,
          ...
          work_post = (nacp_doc.step_1.workPost || "").trim(),
          work_place = (nacp_doc.step_1.workPlace || "").trim(),
          corruption_affected = nacp_doc.step_1.corruptionAffected || "",
          post_category = remove_not_applicable(nacp_doc.step_1.postCategory),
          post_category_extendedstatus = nacp_doc.step_1.postCategory_extendedstatus || "",
          post_type = remove_not_applicable(nacp_doc.step_1.postType),
          public_person = remove_not_applicable(nacp_doc.step_1.public_person),
    ...
    emit(doc._id,
         [doc.doc_uuid, dcu_link, full_name, doc.intro.declaration_year, name_post,
